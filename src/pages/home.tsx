...
// ✅ Jackpot suspense settle (mobile-safe: runs AFTER compaction)
const JP_SNAP_EDGE_CHANCE = 0.32; // chance to do the off-center fake land
const JP_SNAP_EDGE_FRAC_MIN = 0.22; // fraction of a tile (WHEEL_STEP)
const JP_SNAP_EDGE_FRAC_MAX = 0.34;
const JP_SNAP_PAUSE_MS = 160; // pause before snapping to center
const JP_SNAP_SETTLE_MS = 320; // snap duration
const JP_POP_HOLD_MS = 1200; // how long the winner stays popped
              ...
              <div
                key={slowMode ? `${it.key}__dup_${idx}` : it.key}
                  ...
                  isWinner ? "jpWheelItemWinner" : ""} ${
                   isCenterWinner && winnerPopActive ? "jpWheelItemWinnerPop" : ""
...
const wheelStopIndexRef = useRef<number>(-1);
  ...
  // ✅ Winner tile pop (used for suspense settle after compaction)
  const [winnerPopActive, setWinnerPopActive] = useState<boolean>(false);
  const winnerPopTimerRef = useRef<any>(null);
  const jpSnapDoneRef = useRef<Set<string>>(new Set());

  function cancelWinnerPop() {
    if (winnerPopTimerRef.current) {
      clearTimeout(winnerPopTimerRef.current);
      winnerPopTimerRef.current = null;
    }
    setWinnerPopActive(false);
  }
...
for (let k = 0; k < tailCount; k++) {
  ...
  function compactReelForResult(roundId: string) {
    ...
    // ✅ suspense settle (mobile-safe): off-center "fake land" then snap to true center
    if (wheelModeRef.current === "RESULT" && !jpSnapDoneRef.current.has(rid)) {
      jpSnapDoneRef.current.add(rid);

      const doEdge = Math.random() < JP_SNAP_EDGE_CHANCE;
      if (doEdge) {
        const frac =
          JP_SNAP_EDGE_FRAC_MIN +
          Math.random() * (JP_SNAP_EDGE_FRAC_MAX - JP_SNAP_EDGE_FRAC_MIN);
        const sign = Math.random() < 0.5 ? -1 : 1;
        const offsetPx = sign * frac * WHEEL_STEP;

        // show edge land instantly (no transition)
        setWheelTransition("none");
        setWheelTranslate(newTranslate + offsetPx);

        // pause, then snap back to true center + pop winner tile
        if (winnerPopTimerRef.current) clearTimeout(winnerPopTimerRef.current);
        winnerPopTimerRef.current = setTimeout(() => {
          if (wheelModeRef.current !== "RESULT") return;

          setWinnerPopActive(true);

          requestAnimationFrame(() => {
            // force layout to avoid iOS flicker
            try {
            ...
            requestAnimationFrame(() => {
              setWheelTransition(
                `transform ${JP_SNAP_SETTLE_MS}ms cubic-bezier(0.16, 0.95, 0.12, 1)`
              );
              setWheelTranslate(newTranslate);

              // drop pop after a beat
              if (winnerPopTimerRef.current) clearTimeout(winnerPopTimerRef.current);
              winnerPopTimerRef.current = setTimeout(() => {
                setWinnerPopActive(false);
              }, JP_POP_HOLD_MS);
      ...
      <div className="spValue" style={{ fontSize: 16 }}>
        {cumJp2Near}
              ...
              <JackpotWheel
                ...
                winnerPopActive={winnerPopActive}
