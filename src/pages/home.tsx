...
// ✅ Suspense landing: sometimes stop slightly off-center then snap back to center
const WHEEL_LAND_EDGE_CHANCE = 0.35;      // how often we "land near edge" first
const WHEEL_LAND_CENTER_FRAC = 0.08;      // normal small offset range (±8% of a tile)
const WHEEL_LAND_EDGE_FRAC_MIN = 0.22;    // edge offset range min (22% of a tile)
const WHEEL_LAND_EDGE_FRAC_MAX = 0.34;    // edge offset range max (34% of a tile)
const WHEEL_LAND_PAUSE_MS = 160;          // pause after landing before snapping to center
const WHEEL_LAND_SETTLE_MS = 380;         // snap-to-center duration
...
const wheelStopIndexRef = useRef<number>(-1);
  ...
  // ✅ landing suspense phase: MAIN -> SETTLE -> DONE (prevents flashing + adds snap-back suspense)
  const wheelLandPhaseRef = useRef<"MAIN" | "SETTLE" | "DONE">("DONE");
...
for (let k = 0; k < tailCount; k++) {
  ...
  reel.push({
    ...
    // ✅ suspense: land a bit off-center, then snap to perfect center on transitionend
    wheelLandPhaseRef.current = "MAIN";
    wheelCenterTranslateRef.current = centerTranslate;

    const edge = Math.random() < WHEEL_LAND_EDGE_CHANCE;
    const dir = Math.random() < 0.5 ? -1 : 1;

    const frac = edge
      ? WHEEL_LAND_EDGE_FRAC_MIN +
        Math.random() * (WHEEL_LAND_EDGE_FRAC_MAX - WHEEL_LAND_EDGE_FRAC_MIN)
      : Math.random() * WHEEL_LAND_CENTER_FRAC;

    const offsetPx = dir * frac * WHEEL_STEP;
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        setWheelTranslate(mainEndTranslate);
      });
    });
  }
  ...
  // ✅ Suspense settle: MAIN (off-center) -> SETTLE (snap to center) -> DONE (then finalize)
  if (wheelMode === "SPIN") {
    if (wheelLandPhaseRef.current === "MAIN") {
      wheelLandPhaseRef.current = "SETTLE";

      // keep transform stable (mobile Safari), then snap to center after a tiny pause
      setWheelTransition("none");

      // force a frame boundary + reflow before we re-enable transition
      requestAnimationFrame(() => {
        try {
          wheelWrapRef.current?.getBoundingClientRect();
          (wheelWrapRef.current as any)?.offsetWidth;
        } catch {}

        setTimeout(() => {
          requestAnimationFrame(() => {
            setWheelTransition(
              `transform ${WHEEL_LAND_SETTLE_MS}ms cubic-bezier(0.16, 0.95, 0.12, 1)`
            );
            setWheelTranslate(wheelCenterTranslateRef.current);
          });
        }, WHEEL_LAND_PAUSE_MS);
      });
    ...
    if (wheelLandPhaseRef.current === "SETTLE") {
      wheelLandPhaseRef.current = "DONE";
      // fall through: this transitionend is the "snap to center" completion
    }
  }
